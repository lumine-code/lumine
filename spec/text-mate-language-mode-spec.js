...
describe("TextMateLanguageMode", () => {
  ...
  beforeEach(async () => {
    ...
    config.set("editor.useTreeSitterParsers", false);
  ...
  afterEach(() => {
    ...
    config.unset("editor.useTreeSitterParsers");
  });
  ...
  describe("tokenizing", () => {
    ...
    describe("when the grammar is updated because a grammar it includes is activated", () => {
      ...
      it("retokenizes the buffer", () => {
        ...
        lumine.grammars.loadGrammarSync(require.resolve("./fixtures/grammars/includer.json"));

        buffer = lumine.project.bufferForPathSync();
        buffer.setText("SENTINEL");

        languageMode = new TextMateLanguageMode({
          buffer,
          config,
          grammar: lumine.grammars.grammarForScopeName("source.includer"),
        });
        ...
        lumine.grammars.loadGrammarSync(require.resolve("./fixtures/grammars/included.json"));
        fullyTokenize(languageMode);
    ...
    describe("when the buffer is configured with the null grammar", () => {
      it("does not actually tokenize using the grammar", () => {
        ...
        buffer = lumine.project.bufferForPathSync("sample.will-use-the-null-grammar");
        buffer.setText("a\nb\nc");
  ...
  describe(".bufferRangeForScopeAtPosition(selector, position)", () => {
    ...
    describe("when the selector does not match the token at the position", () =>
      it("returns a falsy value", () =>
        expect(languageMode.bufferRangeForScopeAtPosition(".bogus", [0, 1])).toBeUndefined()));

    describe("when the selector matches a single token at the position", () => {
      it("returns the range covered by the token", () => {
        expect(languageMode.bufferRangeForScopeAtPosition(".storage.type.var.js", [0, 1])).toEqual([
          [0, 0],
          [0, 3],
        ]);
        expect(languageMode.bufferRangeForScopeAtPosition(".storage.type.var.js", [0, 3])).toEqual([
          [0, 0],
          [0, 3],
        ]);
      });
    ...
    describe("when the selector matches a run of multiple tokens at the position", () => {
      it("returns the range covered by all contiguous tokens (within a single line)", () => {
        expect(languageMode.bufferRangeForScopeAtPosition(".function", [1, 18])).toEqual([
          [1, 6],
          [1, 28],
        ]);
      });
  ...
  describe(".buildHighlightIterator", () => {
    ...
    it("iterates over the syntactic scope boundaries", () => {
      ...
      expect(
        iterator.seek(Point(0, 1)).map((scopeId) => languageMode.classNameForScopeId(scopeId)),
      ).toEqual([
        "syntax--source syntax--js",
        "syntax--storage syntax--type syntax--var syntax--js",
      ]);
      ...
      expect(
        iterator.seek(Point(0, 8)).map((scopeId) => languageMode.classNameForScopeId(scopeId)),
      ).toEqual(["syntax--source syntax--js"]);
      expect(iterator.getPosition()).toEqual(Point(0, 8));
      expect(
        iterator.seek(Point(1, 0)).map((scopeId) => languageMode.classNameForScopeId(scopeId)),
      ).toEqual(["syntax--source syntax--js", "syntax--comment syntax--block syntax--js"]);
      expect(iterator.getPosition()).toEqual(Point(1, 0));
      expect(
        iterator.seek(Point(1, 18)).map((scopeId) => languageMode.classNameForScopeId(scopeId)),
      ).toEqual([
        "syntax--source syntax--js",
      ...
      expect(
        iterator.seek(Point(2, 0)).map((scopeId) => languageMode.classNameForScopeId(scopeId)),
      ).toEqual(["syntax--source syntax--js"]);
      iterator.moveToSuccessor();
    ...
    it("correctly terminates scopes at the beginning of the line (regression)", () => {
      const grammar = lumine.grammars.createGrammar("test", {
      ...
      expect(
        iterator.getCloseScopeIds().map((scopeId) => languageMode.classNameForScopeId(scopeId)),
      ).toEqual(["syntax--blue syntax--broken"]);
      expect(
        iterator.getOpenScopeIds().map((scopeId) => languageMode.classNameForScopeId(scopeId)),
  ...
  describe(".getFoldableRanges", () => {
    ...
    it("works with multi-line comments", async () => {
      await lumine.packages.activatePackage("language-javascript");
      const editor = await lumine.workspace.open("sample-with-comments.js", {
        autoIndent: false,
      });
      fullyTokenize(editor.getBuffer().getLanguageMode());
  ...
  describe(".getFoldableRangeContainingPoint", () => {
    ...
    it("works for python", async () => {
      ...
      expect(languageMode.getFoldableRangeContainingPoint(Point(0, Infinity), 2)).toEqual([
        [0, Infinity],
        [20, Infinity],
      ]);
      expect(languageMode.getFoldableRangeContainingPoint(Point(1, Infinity), 2)).toEqual([
        [1, Infinity],
        [17, Infinity],
      ]);
      expect(languageMode.getFoldableRangeContainingPoint(Point(2, Infinity), 2)).toEqual([
        [1, Infinity],
        [17, Infinity],
      ]);
      expect(languageMode.getFoldableRangeContainingPoint(Point(19, Infinity), 2)).toEqual([
        [19, Infinity],
        [20, Infinity],
      ]);
    });
    ...
    it("works for javascript", async () => {
      ...
      expect(editor.languageMode.getFoldableRangeContainingPoint(Point(0, Infinity), 2)).toEqual([
        [0, Infinity],
        [12, Infinity],
      ]);
      expect(editor.languageMode.getFoldableRangeContainingPoint(Point(1, Infinity), 2)).toEqual([
        [1, Infinity],
        [9, Infinity],
      ]);
      expect(editor.languageMode.getFoldableRangeContainingPoint(Point(2, Infinity), 2)).toEqual([
        [1, Infinity],
        [9, Infinity],
      ]);
      expect(editor.languageMode.getFoldableRangeContainingPoint(Point(4, Infinity), 2)).toEqual([
        [4, Infinity],
        [7, Infinity],
      ]);
    });
