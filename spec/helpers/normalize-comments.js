...
async function runGrammarTests(fullPath, commentRegex, trailingCommentRegex = null) {
  ...
  expect(normalized.length).toBeGreaterThan(
    0,
    "Tokenizer didn't run correctly - could not find any comment",
  );
...
async function runFoldsTests(fullPath, commentRegex) {
  ...
  normalizeTreeSitterTextData(editor, commentRegex).forEach((test) => {
    ...
    if (!kind || !id) {
      throw new Error(
        `Folds must be in the format fold_end.some-id\n` +
  ...
  for (const k in grouped) {
    ...
    if (keys.indexOf("fold_begin") === -1) throw new Error(`Fold ${k} must contain fold_begin`);
    if (keys.indexOf("fold_end") === -1) throw new Error(`Fold ${k} must contain fold_end`);
  ...
  for (const k in grouped) {
    ...
    expect(editor.isFoldableAtBufferRow(begin.editorPosition.row)).toSatisfy((foldable, reason) => {
      reason(
        `Editor is not foldable at row ${begin.editorPosition.row + 1}\n` +
    ...
    expect(editor.screenPositionForBufferPosition(end.editorPosition)).toSatisfy(
      (screenPosition, reason) => {
        const { row, column } = newPos.editorPosition;
        reason(
          `At row ${begin.editorPosition.row + 1}, editor should fold ` +
