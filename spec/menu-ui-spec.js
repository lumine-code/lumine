...
describe("HTML menu UI", () => {
  ...
  it("does not rearm the MenuBar when Alt repeats after consumed wheel input", () => {
    const emitter = new Emitter();
    const fakeManager = {
      getTemplate: () => [{ label: "&File", submenu: [{ label: "Open", command: "core:open" }] }],
      onDidChange: (callback) => emitter.on("change", callback),
    ...
    const menuBar = new MenuBarView(fakeManager, { autoHide: true, altGivesFocus: true });
    const wheelTarget = document.createElement("div");
    wheelTarget.addEventListener("wheel", (event) => event.stopPropagation());
    document.body.append(menuBar.element, wheelTarget);

    document.body.dispatchEvent(
      new KeyboardEvent("keydown", { key: "Alt", altKey: true, bubbles: true }),
    );
    wheelTarget.dispatchEvent(new WheelEvent("wheel", { altKey: true, bubbles: true }));
    document.body.dispatchEvent(
      new KeyboardEvent("keydown", {
        key: "Alt",
        altKey: true,
