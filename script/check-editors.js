...
// says so on the element: `atom-text-editor[input]`, which `[mini]` implies.
// Themes used to guess instead, with `atom-panel-container atom-text-editor`
...
// reaches an `atom-text-editor` through a panel container. Structural panel
...
function violates(chain) {
  const gated = chain.some((link) => /atom-panel-container/.test(link));
  if (!gated) return false;
  return chain.some((link) => /atom-text-editor/.test(link));
}
      ...
      for (const chain of rules(css)) {
        ...
        errors.push(
          `${path.relative(ROOT, file)}: "${chain.filter(Boolean).join(" ")}" reaches an ` +
            `editor through a panel container — style atom-text-editor[input] instead, ` +
