...
module.exports = class ProviderConfig {
  constructor(options = {}) {
  ...
  scopeDescriptorToType(scopeDescriptor) {
    ...
    for (const type of Object.keys(config)) {
      ...
      if (typePriority > highestTypePriority && selectorsMatchScopeChain(selectors, scopeChain)) {
        matchingType = type;
    ...
    if (suggestions != null && Array.isArray(suggestions)) {
      ...
      for (let i = 0; i < suggestions.length; i++) {
        ...
        if (typeof suggestion === "string") {
          ...
          (suggestion.text != null || suggestion.snippet != null)
        ) {
