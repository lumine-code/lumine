...
module.exports = {
  ...
  isCompletingValue({ scopeDescriptor, bufferPosition, prefix, editor }) {
    ...
    const beforePrefixBufferPosition = [
      bufferPosition.row,
      Math.max(0, bufferPosition.column - prefix.length - 1),
    ];
    ...
    const inMetaPropertyList =
      hasScope(scopes, "meta.property-list", ["css", "scss", "postcss"]) && prefix.trim() === ":";
    const inMetaSelector =
      hasScope(scopes, "meta.block.inside-selector", ["css", "scss", "postcss"]) &&
      prefix.trim() === ":";
    ...
    return (
      ...
      (hasScope(scopes, "source.sass") &&
        (hasScope(scopes, "meta.property-value.sass") ||
  ...
  isCompletingName({ scopeDescriptor, bufferPosition, prefix, editor }) {
    ...
    const isInPropertyList =
      !isAtTerminator &&
      (hasScope(scopes, "meta.property-list") || hasScope(scopes, "meta.block.inside-selector"));
    ...
    if (
      hasScope(previousScopesArray, "entity.other.attribute-name.class") ||
    ...
    const isAtBeginScopePunctuation =
      hasScope(scopes, "punctuation.section.property-list.begin", ["css", "scss", "postcss"]) ||
      hasScope(scopes, "punctuation.definition.property-list.begin", ["css", "scss", "postcss"]);

    const isAtEndScopePunctuation =
      hasScope(scopes, "punctuation.section.property-list.end", ["css", "scss", "postcss"]) ||
      hasScope(scopes, "punctuation.definition.property-list.end", ["css", "scss", "postcss"]);
  ...
  isCompletingNameOrTag({ scopeDescriptor, bufferPosition, editor }) {
    ...
    return (
      this.isPropertyNamePrefix(prefix) &&
  ...
  isCompletingTagSelector({ editor, scopeDescriptor, bufferPosition }) {
    ...
    if (!(tagSelectorPrefix != null ? tagSelectorPrefix.length : undefined)) {
      return false;
    }
    ...
    if (
      hasScope(scopes, "meta.selector.css") ||
      hasScope(previousScopesArray, "meta.selector.css")
    ) {
      return true;
    } else if (
      hasScope(scopes, "source.css", ["scss", "less", "postcss"]) ||
      hasScope(scopes, "source.css")
    ) {
      return (
        !hasScope(previousScopesArray, "meta.property-value", ["scss", "css", "postcss"]) &&
  ...
  isCompletingPseudoSelector({ editor, scopeDescriptor, bufferPosition }) {
    ...
    if (
      ...
      if (prefix) {
        ...
        return (
          !hasScope(previousScopesArray, "meta.property-name.scss") &&
  ...
  getImportantPrefix(editor, bufferPosition) {
    ...
    return typeof importantPrefixPatternExec !== "undefined" && importantPrefixPatternExec !== null
      ? importantPrefixPatternExec[1]
      : undefined;
  },
  ...
  getPreviousPropertyName(bufferPosition, editor) {
    ...
    while (row >= 0) {
      ...
      let propertyName =
        typeof inlinePropertyNameWithColonPattern.exec(line) !== "undefined" &&
        inlinePropertyNameWithColonPattern.exec(line) !== null
          ? inlinePropertyNameWithColonPattern.exec(line)[1]
          : undefined;
      if (propertyName == null) {
        propertyName =
          typeof firstInlinePropertyNameWithColonPattern.exec(line) !== "undefined" &&
          firstInlinePropertyNameWithColonPattern.exec(line) !== null
            ? firstInlinePropertyNameWithColonPattern.exec(line)[1]
            : undefined;
      }
      if (propertyName == null) {
        propertyName =
          typeof propertyNameWithColonPattern.exec(line) !== "undefined" &&
          propertyNameWithColonPattern.exec(line) !== null
  ...
  getPropertyValueCompletions({ bufferPosition, editor, prefix, scopeDescriptor }) {
    ...
    const scopes = scopeDescriptor.getScopesArray();
    const addSemicolon =
      !lineEndsWithSemicolon(bufferPosition, editor) && !hasScope(scopes, "source.sass");
    ...
    } else if (!hasScope(scopes, "keyword.other.unit.percentage.css")) {
      // CSS
  ...
  getPropertyNamePrefix(bufferPosition, editor) {
    const line = editor.getTextInRange([[bufferPosition.row, 0], bufferPosition]);
    return typeof propertyNamePrefixPattern.exec(line) !== "undefined" &&
      propertyNamePrefixPattern.exec(line) !== null
  ...
  getPseudoSelectorPrefix(editor, bufferPosition) {
    const line = editor.getTextInRange([[bufferPosition.row, 0], bufferPosition]);
    return typeof line.match(pseudoSelectorPrefixPattern) !== "undefined" &&
      line.match(pseudoSelectorPrefixPattern) !== null
  ...
  getTagSelectorPrefix(editor, bufferPosition) {
    const line = editor.getTextInRange([[bufferPosition.row, 0], bufferPosition]);
    return typeof tagSelectorPrefixPattern.exec(line) !== "undefined" &&
      tagSelectorPrefixPattern.exec(line) !== null
...
const hasScope = function (scopesArray, scope, endsWith = null) {
  ...
  for (var otherScope of Array.from(scopesArray)) {
    if (endsWith && Array.isArray(endsWith)) {
      if (!endsWith.some((ending) => otherScope.endsWith(`.${ending}`))) {
        continue;
      }
    }
    if (otherScope === scope) {
    ...
    if (otherScope.startsWith(`${scope}.`)) {
      return true;
    }
  }
};
