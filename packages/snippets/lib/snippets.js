...
function showCommandNameConflictNotification(name, commandName, packageName, snippetsPath) {
  ...
  atom.notifications.addError(`Snippets conflict`, {
    description: message,
    dismissable: true,
  });
}
...
function showInvalidCommandNameNotification(name, commandName) {
  ...
  atom.notifications.addError(`Snippets error`, {
    description: message,
    dismissable: true,
  });
}
...
module.exports = {
  activate() {
    ...
    this.subscriptions.add(
      atom.config.onDidChange("core.packagesWithSnippetsDisabled", ({ newValue, oldValue }) => {
    ...
    fs.stat(userSnippetsPath, (_error, stat) => {
      if (stat != null && stat.isFile()) {
        ...
        try {
          ...
          userSnippetsFileDisposable.add(
            userSnippetsFile.onDidDelete(() => this.handleUserSnippetsDidChange()),
          );
          userSnippetsFileDisposable.add(
  ...
  loadSnippetsFile(filePath, callback) {
    ...
    CSON.readFile(filePath, { allowDuplicateKeys: false }, (error, object = {}) => {
      if (error != null) {
        console.warn(
          `Error reading snippets file '${filePath}': ${error.stack != null ? error.stack : error}`,
        );
  ...
  addCommandForSnippet(attributes, packageName, _selector) {
    ...
    let disposable = atom.commands.add("atom-text-editor", commandName, commandHandler);
  ...
  getScopeChain(object) {
    ...
    return scopesArray.map((scope) => (scope[0] === "." ? scope : `.${scope}`)).join(" ");
  },
  ...
  parsedSnippetsForScopes(scopeDescriptor) {
    ...
    if (unparsedSnippetsByPrefix) {
      for (const prefix in unparsedSnippetsByPrefix) {
        ...
        if (typeof (attributes != null ? attributes.body : undefined) !== "string") {
          continue;
        }
  ...
  textChanged(editor, event) {
    ...
    this.ignoringTextChangesForEditor(editor, () =>
      editor.transact(() => activeExpansions.map((expansion) => expansion.textChanged(event))),
    );
