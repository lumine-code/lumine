...
module.exports = class SnippetExpansion {
  ...
  constructor(snippet, editor, cursor, snippets, { method, adjustIndentation = true } = {}) {
    ...
    this.ignoringBufferChanges(() => {
      this.editor.transact(() => {
        ...
        const newRangeMarker = this.getMarkerLayer(this.editor).markBufferRange(newRange, {
          exclusive: false,
        });
  ...
  applyAllTransformations() {
    this.editor.transact(() => {
      this.insertionsByIndex.forEach((insertion, index) => this.applyTransformations(index));
    });
  }
  ...
  markVariables(startPosition) {
    ...
    for (const variable of this.snippet.variables) {
      ...
      let markerRange = new Range(startPosition.translate(point), startPosition.translate(point));
      const marker = this.getMarkerLayer(this.editor).markBufferRange(markerRange, {
        exclusive: false,
      });
    ...
    for (let [index, tabStop] of tabStops.entries()) {
      ...
      for (const insertion of insertions) {
        ...
        const marker = this.getMarkerLayer(this.editor).markBufferRange(
          [startPosition.traverse(start), startPosition.traverse(end)],
          { exclusive: false },
        );
    ...
    if (ranges.length > 0) {
      ...
      for (let i = 0; i < ranges.length; i++) {
        ...
        if (this.selections[i]) {
          ...
          this.subscriptions.add(
            newSelection.cursor.onDidChangePosition((event) => this.cursorMoved(event)),
          );
