...
module.exports = class BracketMatcherView {
  constructor(editor, editorElement, matchManager) {
    ...
    this.subscriptions.add(
      ...
      atom.commands.add(editorElement, "bracket-matcher:close-tag", () => this.closeTag()),
  ...
  findMatchingEndBracket(startBracketPosition, startBracket, endBracket) {
    ...
    if (this.hasSyntaxTree()) {
      return this.findMatchingEndBracketWithSyntaxTree(
      ...
      return this.findMatchingEndBracketWithRegexSearch(
        startBracketPosition,
        startBracket,
        endBracket,
      );
  ...
  findMatchingStartBracket(endBracketPosition, startBracket, endBracket) {
    ...
    if (this.hasSyntaxTree()) {
      return this.findMatchingStartBracketWithSyntaxTree(
        endBracketPosition,
        startBracket,
        endBracket,
      );
      ...
      if (this.isScopeCommentedOrString(scopeDescriptor.getScopesArray())) return;
      return this.findMatchingStartBracketWithRegexSearch(
        endBracketPosition,
        startBracket,
        endBracket,
      );
  ...
  findMatchingEndBracketWithSyntaxTree(bracketPosition, startBracket, endBracket) {
    ...
    this.editor.buffer
      .getLanguageMode()
      .getSyntaxNodeContainingRange(new Range(bracketPosition, bracketEndPosition), (node) => {
        if (
          bracketEndPosition.isGreaterThan(node.startPosition) &&
          bracketEndPosition.isLessThan(node.endPosition)
        ) {
          const matchNode = node.children.find(
            (child) =>
  ...
  findMatchingTagNameRangesWithSyntaxTree() {
    ...
    if (
      startTag &&
      (startTag.range.containsPoint(position) || endTag.range.containsPoint(position))
    ) {
      if (startTag === endTag) {
  ...
  findMatchingEndBracketWithRegexSearch(startBracketPosition, startBracket, endBracket) {
    ...
    this.editor.scanInBufferRange(
      this.matchManager.pairRegexes[startBracket],
      scanRange,
      (result) => {
  ...
  createMarker(bufferRange) {
    ...
    if (
      atom.config.get("bracket-matcher.highlightMatchingLineNumber", {
  ...
  findCurrentPair() {
    ...
    const currentCharacter = this.editor.getTextInBufferRange(
      new Range(currentPosition, nextPosition),
    );
    const previousCharacter = this.editor.getTextInBufferRange(
      new Range(previousPosition, currentPosition),
    );
  ...
  goToMatchingBracket() {
    ...
    if (this.tagHighlighted) {
      ...
      startRange = new Range(
        startRange.start.traverse(ONE_CHAR_BACKWARD_TRAVERSAL),
        endRange.end.traverse(ONE_CHAR_BACKWARD_TRAVERSAL),
      );
      // include the </
      endRange = new Range(
        endRange.start.traverse(TWO_CHARS_BACKWARD_TRAVERSAL),
        endRange.end.traverse(TWO_CHARS_BACKWARD_TRAVERSAL),
      );
  ...
  multiCursorSelect() {
    this.editor.getCursorBufferPositions().forEach((position) => {
      ...
      if (startPosition) {
        const startBracket = this.editor.getTextInRange(
          Range.fromPointWithDelta(startPosition, 0, 1),
        );
        const endPosition = this.findMatchingEndBracket(
          startPosition,
          startBracket,
          this.matchManager.pairedCharacters[startBracket],
        );
  ...
  isCursorOnCommentOrString() {
    return this.isScopeCommentedOrString(
      this.editor.getLastCursor().getScopeDescriptor().getScopesArray(),
    );
  }
