...
module.exports = class NotificationIssue {
  ...
  findSimilarIssues() {
    ...
    return fetch(
      `https://api.github.com/search/issues?q=${encodeURIComponent(query)}&sort=created`,
  ...
  getIssueUrlForSystem() {
    ...
    return this.getIssueUrl().then((issueUrl) =>
...
`;
  ...
  normalizedStackPaths(stack) {
    return stack != null
      ? stack.replace(
          /(^\W+at )([\w.]{2,} [(])?(.*)(:\d+:\d+[)]?)/gm,
          (m, p1, p2, p3, p4) => p1 + (p2 || "") + this.normalizePath(p3) + p4,
        )
      : undefined;
  }
  ...
  getRepoUrl() {
    ...
    let repoUrl =
      (repo != null ? repo.url : undefined) != null ? (repo != null ? repo.url : undefined) : repo;
    if (!repoUrl) {
      let packagePath;
      if ((packagePath = atom.packages.resolvePackagePath(packageName))) {
        try {
          ...
          repoUrl =
            (repo != null ? repo.url : undefined) != null
              ? repo != null
                ? repo.url
                : undefined
              : repo;
  ...
  getPackageName() {
    ...
    if (options.stack != null) {
      ...
      for (
        let i = 0, end = stack.length, asc = 0 <= end;
        asc ? i < end : i > end;
        asc ? i++ : i--
      ) {
        var { file } = stack[i];
