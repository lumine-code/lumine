const Node = require("./line-top-index-node");
const { ZERO_POSITION, add: addLogicalPositions } = require("./logical-position-helpers");
...
class Iterator {
  ...
  insertNode(row, matchNodesAtSameRow = true) {
    this.reset();
    ...
    while (true) {
      ...
      if (blockStart <= pixelPosition && pixelPosition <= blockEnd) {
        ...
        if (this.currentNode.left) {
          ...
          let previousBlockEnd =
            this.leftAncestorPosition.row * lineHeight + this.leftAncestorPosition.pixels;
  ...
  setCurrentNode(node) {
    ...
    if (this.currentNode) {
      this.currentPosition = addLogicalPositions(
        this.leftAncestorPosition,
        this.currentNode.distanceFromLeftAncestor,
      );
    }
  }
